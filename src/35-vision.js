...
const FINAL_FRAG = `
  ...
  void main() {
    ...
    // Weighted towards the periphery, but with a real floor at the centre. The
    // first version was 0.45 + 1.9*r2, which put nearly all of it out at the
...
const DOSE_AT_START = 0.30;      // present from the first room, not merely non-zero
...
let _pinned = null;
function pinDose(v) { _pinned = (v === null || v === undefined) ? null : Math.max(0, Math.min(1, v)); }

function updateDose(spaceKey, dt) {
  if (_pinned !== null) { dose = _pinned; return dose; }
