...
const Music = (() => {
  ...
  let throbBus, padBus, metalBus, bedBus, bedGate;
  ...
  function throb(t, level, hard) {
    ...
    const sh = shaper(2.2 + 5 * M.depth + 3 * M.unease);
    const env = ctx.createGain(); env.gain.value = 0.0001;

    o.connect(flt); w.connect(wg); wg.connect(flt);
    flt.connect(sh); sh.connect(env); env.connect(throbBus);
  ...
  function swell(notes_, t, attack, hold, release, level, cutoff) {
    ...
    notes_.forEach((midi, i) => {
      ...
      for (const [amp, mult] of parts) {
        // three, not two, and one of them a long way out of tune: two voices
        // beat, three grind
        for (const det of [-5, 6, 17 * (0.4 + M.unease)]) {
  ...
  function stab(base, t, level) {
    for (const midi of [base, base + (mchance(0.5) ? 1 : 6), base + 11, base + 12]) {
      ...
      const sh = shaper(4);
      const env = ctx.createGain(); env.gain.value = 0.0001;
      a.connect(flt); b.connect(flt); flt.connect(sh); sh.connect(env); env.connect(metalBus);
  ...
  function buildBed() {
    ...
    const amp2 = ctx.createOscillator(); amp2.type = 'triangle'; amp2.frequency.value = 0.31;
  ...
  function step(n, t) {
    ...
    if (bar === 0) {
      ...
      if (wantChord !== chord) { chord = wantChord; soundChord(t); }
