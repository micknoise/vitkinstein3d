...
const Music = (() => {
  ...
  let throbBus, padBus, metalBus, bedBus, bedGate, throbDrive;
  let throbMakeup, gritOut;
  let bedNodes = null, carried = null, glideUntil = 0, glideAt = 1e9, _ir = null;
  let analyser = null, anBuf = null, loud = 0, quietFor = 0, lastDrop = -99;
  const drops = [];
  // ?noverb=1 takes the convolution out and leaves everything else exactly as
  // it is. Reported dropouts are believed to be the reverb; this is the one
  // experiment that settles it, and it has to be run on the machine it happens
  // on rather than on mine.
  let NO_VERB = false;
  ...
  let chord = 0, wantChord = 0, swellDue = false;
  ...
  function buildBus(destination) {
    ...
    // --- catching a dropout in the act -----------------------------------
    // I cannot reproduce the remaining ones here, so this listens to the
    // score's own output on the machine where they happen. Every frame it
    // takes the output's level; when the level collapses while the score
    // believes it is playing, it writes down what was going on at that moment.
    // VK.music.drops is the list, and each one is also warned to the console.
    ...
    duck.connect(bus);
    tilt.connect(comp); comp.connect(ceiling);
    ...
    conv.normalize = false;                 // the level is ours, not the browser's
    ...
    revReturn = ctx.createGain(); revReturn.gain.value = NO_VERB ? 0 : 1.15;
    ...
    // and one dark feedback delay, off the metal only
    ...
    // One shaper for the throb, built once. The first
  ...
  //
  // The first version of this was wrong in a way that does not show up as a
  // bad noise, which is the worst kind. Darkening the tail was done with a
  // one-pole lowpass whose corner falls towards the end, and a one-pole
  // lowpass run over noise is an *integrator*: what came out was a random
  // walk, with big slow excursions and a real DC component. Measured on the
  // response it produced, the sum of its samples was 305 against an expected
  // mid-band magnitude of about 47 -- so the reverb had roughly sixteen dB
  // more gain at DC than at the frequencies it was supposed to be reverberating,
  // and its tail wandered up and down at frequencies nobody can hear on a
  // signal that was mostly bass to begin with.
  //
  // Three things fix it, and all three are ordinary practice. The mean is
  // taken out. A one-pole highpass at 90Hz goes over the finished response, so
  // there is no DC and no infrasonic wander left in it at all. And it is
  // normalised to unit energy with `conv.normalize` turned *off*, so the gain
  // of the reverb is a number chosen here rather than whatever the browser's
  // own normalisation makes of a buffer whose character keeps changing.
  //
  // It is also mono and 3.2 seconds rather than stereo and 6.0, which is a
  // quarter of the work on the audio thread for a tail nobody was hearing the
  // end of.
  function impulse(sec, decay) {
    ...
    for (let k = 0; k < 9; k++) {
      ...
      if (at < len) d[at] += (MR() * 2 - 1) * 0.42 * (1 - k / 9);
    }

    // no mean, and nothing under 90Hz
    let mean = 0;
    ...
    // unit energy, so the convolution is about unity gain for anything
    // broadband and the level is set by revReturn and nothing else
    let sq = 0;
    for (let i = 0; i < len; i++) sq += d[i] * d[i];
    const g = 1 / Math.sqrt(Math.max(1e-12, sq));
  ...
  // cancelScheduledValues removes the events at or after a time -- and a
  // parameter in the middle of a ramp then reverts to the last value set
  // *before* it, which for an organ still in its attack is the 0.0001 it
  // started from. So killing a swell before it had finished arriving did not
  // fade it, it cut it to silence in one sample, with the click that implies.
  // Reported from play as the pad being cut off when you leave a room, which
  // is exactly when it happens: rooms change faster than a 2.4 second attack.
  //
  // cancelAndHoldAtTime keeps the value the parameter has actually reached and
  // lets the fade start from there. Every place in here that interrupts a ramp
  // goes through this.
  function holdAt(param, t) {
    if (param.cancelAndHoldAtTime) param.cancelAndHoldAtTime(t);
    else param.cancelScheduledValues(t);          // older engines: no hold
  }

  function killPads(t, fade) {
    for (const v of padVoices) {
      for (const e of v.env) { holdAt(e.gain, t); e.gain.setTargetAtTime(0.0001, t, fade / 3); }
  ...
  function flickerBurst(t, dur) {
    ...
    let at = t;
    holdAt(g, t);
    // Two milliseconds, not none. setValueAtTime steps a gain instantaneously,
    // and stepping a sustained drone's gain from 1 to 0.06 in one sample is a
    // click -- measured as the largest sample-to-sample jump in the whole
    // passage, which is what a click is. Two milliseconds is far too fast to
    // hear as a fade and slow enough not to snap.
    while (at < t + dur) {
      const on = mchance(0.45);
      g.linearRampToValueAtTime(on ? 1 : 0.06 + MR() * 0.2, at + 0.002);
  ...
  // The room swallows it for a moment.
  //
  // This was a hole in *everything*, down to 2% for up to a third of a second,
  // across the master -- and a soundtrack that mutes itself for 300ms is
  // indistinguishable from a soundtrack that has broken, which is how it was
  // reported. It now ducks the layers only and not their reverb, so the tail
  // rings on through it: the sound is taken away and the room it was in is
  // not, which is the difference between a held breath and a fault. Shallower
  // and shorter with it.
  function duckFor(t, dur) {
    const g = duck.gain;
    // held, then ramped from wherever it actually is -- not snapped back to 1
    // first, which is a click of its own whenever two of these overlap
    holdAt(g, t);
    g.linearRampToValueAtTime(0.34, t + 0.02);
    g.setValueAtTime(0.34, t + dur);
    g.linearRampToValueAtTime(1, t + dur + 0.12);
  }
  ...
  function step(n, t) {
    ...
    if (bar === 0) {
      ...
      // still the chord the bells are drawn from.
      // A chord change that arrives too soon after the last one does not get
      // its organ -- but it used to be dropped entirely, so the chord moved
      // and the organ went on holding the one before it. Leaving a room and
      // coming back was then a pad that had been cut off, briefly held the
      // wrong chord and then put itself right at whatever the next swell
      // happened to be. It is deferred now, not discarded.
      if (wantChord !== chord) { chord = wantChord; swellDue = true; }
      if (swellDue && t - lastSwellAt > 3.2) { swellDue = false; soundChord(t); }
    ...
    // --- the hole ----------------------------------------------------------
    if (bar === 0 && M.presence > 0.5 && MR() < 0.012 + 0.03 * M.depth)
      duckFor(t + stepDur() * Math.floor(MR() * 4), 0.07 + MR() * 0.11);
  ...
  function fire(e, t) {
    ...
    switch (e.kind) {
      ...
      case 'drop':
        ...
        break;
      case 'throw':
        // No stab. It was the one gesture in here that announced itself, and
        // throwing something is already the loudest thing you can do with your
        // hands -- the object hits a wall a moment later and that lands *in*
        // the bar, which is a better answer than a chord shouting over it.
        dropNote(t);
        break;
      case 'impact': {
  ...
  // Is the score still making a sound? `loud` is the loudest it has been in
  // the last couple of seconds; if the output falls to almost nothing against
  // that while presence says it should be playing, and stays there for a few
  // frames, something has gone wrong that none of the offline measurements
  // catch -- so write down everything that was true at the time.
  function listen(dt) {
    if (!analyser || !anBuf) return;
    analyser.getFloatTimeDomainData(anBuf);
    let sq = 0;
    for (let i = 0; i < anBuf.length; i++) sq += anBuf[i] * anBuf[i];
    const rms = Math.sqrt(sq / anBuf.length);
    loud = Math.max(rms, loud * Math.exp(-dt / 2));
    const playing = M.presence > 0.35 && loud > 0.004;
    if (playing && rms < loud * 0.06) quietFor += dt; else quietFor = 0;
    const now = ctx.currentTime;
    if (quietFor > 0.05 && now - lastDrop > 1.5) {
      lastDrop = now;
      const d = {
        at: +now.toFixed(2), forMs: Math.round(quietFor * 1000),
        rms: +rms.toFixed(5), loud: +loud.toFixed(5),
        motion: +M.motion.toFixed(2), unease: +M.unease.toFixed(2),
        depth: +M.depth.toFixed(2), presence: +M.presence.toFixed(2),
        pads: padVoices.length, squash: +comp.reduction.toFixed(1),
        gliding: now < glideUntil, carrying: !!carried
      };
      drops.push(d);
      if (drops.length > 60) drops.shift();
      console.warn('[score] output collapsed', JSON.stringify(d));
    }
  }
  ...
  function levels(haste) {
    ...
    setg(revReturn, NO_VERB ? 0 : 0.70 + 0.35 * M.depth, 2.0 / haste);
    if (bedNodes) {
  ...
  function render(seconds, state) {
    ...
    const sr = 44100;
    const saved = { ctx, bedNodes, carried, noverb: NO_VERB };
    ...
    try {
      ...
      return ctx.startRendering().then(buf => {
        ...
        // supposed to catch an occasional peak, not to be the loudest thing in the
      ...
      }).finally(() => { ctx = saved.ctx; bedNodes = saved.bedNodes; carried = saved.carried; NO_VERB = saved.noverb; });
    } catch (e) {
      ctx = saved.ctx; bedNodes = saved.bedNodes; carried = saved.carried; NO_VERB = saved.noverb;
