...
// The portal's target holds linear values with no tone mapping -- three skips it
// for off-screen passes. What to do about that depends on where this quad is
// being drawn to.
//
// Normally it is drawn into the scene target, which is *also* linear and gets
// tone-mapped once, at the end, by the vision pass. In that case the honest
// thing is to write the linear value straight through and be tone-mapped along
// with the room around it. Doing anything else here means the view through a
// doorway is developed differently from the doorway, which is exactly what
// playtesters kept seeing: it used to tone-map and gamma-encode its output into
// a linear buffer that then tone-mapped and gamma-encoded it again, on top of
// an exposure of 1.15/0.6 against the room's 1.15. Measured: the view through a
// portal came out 5.3x brighter than the room it was showing.
//
// With ?nofx=1 there is no such pass -- the scene goes straight to the canvas
// and three tone-maps the ordinary materials on the way -- and a ShaderMaterial
// is not one of those, so then it does have to finish the job itself.
const PORTAL_FRAG = `
  ...
  void main() {
    ...
    vec3 lin = texture2D(tex, uv).rgb;
    if (linearOut > 0.5) { gl_FragColor = vec4(lin, 1.0); return; }
    vec3 c = lin * exposure;
...
// Told by initVision(), which is the only thing that knows whether there is a
// pass at the end to do the developing.
function setPortalOutput(linear) {
  for (const p of PORTALS) p.mesh.material.uniforms.linearOut.value = linear ? 1 : 0;
}
...
function makeFace(f) {
  ...
  const mat = new THREE.ShaderMaterial({
    uniforms: { tex: { value: null }, exposure: { value: 1.15 }, linearOut: { value: 0 } },
...
function updatePortals() {
  ...
  for (const p of PORTALS) {
    ...
    // traverse() moves the player and sets every face's side from where they
    // now are, so `pos` and every side computed from it are stale the moment it
    // returns. Carrying on down the list would overwrite those correct flags
    // with ones worked out from where the player used to be, and the next frame
    // would read the difference as another crossing -- of whatever face the
    // jump happened to line up with, however far away it was, because
    // withinFrame only asks whether you are within the doorway's rectangle and
    // not whether you are anywhere near its plane.
    if (p.side > 0 && sign <= 0) { traverse(p); break; }
    p.side = sign;
  }
  updateBodyPortals();
}
...
function renderPortals(visible) {
  ...
  // No shadows in a portal view, and not only because it is three quarters of
  // what the pass costs. The light pool is refilled at *this* camera below, so
  // rendering shadows here bakes the maps against the portal's light positions
  // and the main render then uses them -- every room goes dark. Tried it,
  // measured it, put it back.
