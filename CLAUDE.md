...
`?nomusic=1` (no score), `?noverb=1` (score without its reverb). `?fx=N` scales
what the score does to the picture as well as the pass itself, and is read back
by the score as how much to distort.
