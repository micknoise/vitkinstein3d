# Wittgenstein 3D — experimental plan
...
**Smallest version.** A distorted low throb that only exists while you are
walking, over a drone that goes after sixteen seconds of nothing happening, with
every interaction varying it and every room owning a dissonance.
**Second pass, 2026-08-29.** Played: the arpeggiator had to go altogether, the
result was relaxing rather than disturbing, the drone was not dirty enough,
there was no flicker or suspense, the harmony had unexplained major chords in
it, and nothing had enough reverb. The form was right. See §10.
...
|---|---|---|---|
| 2026-08-31 | **an object flipped as it crossed a portal** | fixed | Reported: *"when objects are carried through a portal, they flip orientation when they pass."* B1 (2026-08-27) taught `traverseBody()` to turn a free-moving object by `dYaw` when it crosses a face, but the sibling path for whatever the *player* is carrying — `traverse()`'s `held` branch — only ever carried position and velocity through the fold, never the quaternion. Most crossings pair two portals facing each other, so `dYaw` is close to a half-turn: the object kept its old world orientation while the camera turned to face the new room around it, which looks exactly like the object spinning 180° in your hands. Same fix as B1, in the branch B1 never touched. Verified headless: an object's orientation relative to the camera before a crossing and one tick after is now within 0.03° (was measuring a genuine ~180° jump at the crossing frame itself, one frame of camera lag behind an already-corrected object — resolved by the next tick). |
| 2026-08-31 | **the mug handle lay flush instead of sticking out** | fixed | Reported: *"the handle is attached to the main cup object at 90 degrees, so that it is flush with the cup, rather than sticking out as a handle would."* A `TorusGeometry`'s ring already lies in the plane a handle needs — radial and vertical, bulging outward from the cylinder wall — so no rotation was needed there at all; the code rotated it 90° about Y regardless, which swings that ring into the depth plane instead, where a handle this thin reads as a sliver pressed against the surface. Rotating about Z instead — to bring the arc's open ends to meet the wall at top and bottom — leaves the ring in the plane it started in. Confirmed by screenshotting the same mug from the same seed before and after: before, a thin dark line flush against the rim; after, a visible loop standing off the side. |
| 2026-08-29 | **the pad was cut off, not faded** | fixed | Played: *"when someone exits a room the pad sound sometimes gets cut off, reappears briefly with the wrong chord and then resets itself."* Two faults, and the first is a Web Audio trap worth writing down. **`cancelScheduledValues(t)` removes the events at or after `t`, and a parameter in the middle of a ramp then reverts to the last value set *before* it** — which for an organ still in its 2.4-second attack is the 0.0001 it started from. So killing a swell to make way for the next one did not fade it, it cut it to silence in a single sample. Rooms change faster than the attack, so leaving one was exactly when it happened. `cancelAndHoldAtTime` keeps the value the parameter has actually reached; every interruption in the score goes through one helper now. The second fault is the wrong chord: a chord change arriving inside the 3.2-second minimum gap between organ swells was **discarded**, so the chord moved and the organ went on holding the one before it until some later event happened to swell. It is deferred now, not dropped. Also, and this is the honest part: **two of the remaining "minor dropouts" were mine.** The score punches a hole in itself for suspense, and that hole was the whole master down to 2% for up to a third of a second — which is indistinguishable from the audio breaking, which is how it was reported. It ducks the layers only now and not their reverb, so the tail rings on through it: the sound is taken away and the room it was in is not. Shallower (34%, not 2%) and shorter with it. The flicker gate was stepping a sustained drone's gain instantaneously, which is a click; two milliseconds, too fast to hear as a fade and slow enough not to snap. And **the stab on throwing is gone** — *"not worth having"* — which leaves a throw silent until the thing you threw hits a wall, and that lands in the bar. What I could not do is reproduce the rest: `VK.music.drops` now listens to the score's own output and writes down everything that was true whenever the level collapses while it believes it is playing, with a console warning each time, because the machine it happens on is not this one. |
| 2026-08-29 | **it was the reverb, and it was the maths** | fixed | *"The dropout is still happening and I am betting that it is the convolution reverb causing it. It sounds like a bug in the maths of the reverb."* Said three times, and right. I had checked the obvious failures and found none — no non-finite samples, no runaway (45s of everything at once holds a steady 0.08–0.12 RMS), and the whole graph renders at 18.6× realtime, so it is not CPU either. The fault was in how the impulse response was drawn. Darkening the tail was done by running a **one-pole lowpass over noise, which is an integrator**: what came out was not a decaying noise burst, it was a **random walk**, with big slow excursions and a real DC term. Measured on the response that shipped: the sum of its samples was 305 against a mean mid-band magnitude of 47 — **sixteen dB more gain at DC than in the band it was supposed to be reverberating**. Everything sent to it came back with a slow infrasonic wander on it, which is inaudible, ate the headroom, and got worse the more was being fed in, which is why it arrived with the intensity and why the glissando — the one voice that sustains for half a minute — was the thing that set it off. Rebuilt, and all three fixes are ordinary practice: the mean is taken out, a 90Hz one-pole highpass goes over the finished response, and it is normalised to unit energy with `conv.normalize` turned **off**, so the gain is a number chosen here rather than whatever the browser makes of a buffer whose character keeps changing. Also mono and 3.2s rather than stereo and 6.0 — a quarter of the work for a tail nobody was hearing the end of. Measured after: DC-to-midband **6.5 → 0.001**, the score's energy under 60Hz **0.70 → 0.39**, graph cost 18.6 → 22.6× realtime. Turning the reverb off now makes the score *darker* rather than brighter, which is the proof: a reverb is supposed to add air, and this one had been adding rumble. **`?noverb=1`** added, because the one experiment I could not run is the one on the machine it happens on. |
| 2026-08-29 | **the score was almost entirely below hearing** | fixed | Reported from play, three times, and I was slow to believe it: *"it drops out when things get too intense... Could it be the convolution just blowing up? Something is definitely overloading to the point of triggering a dropout on all our test machines."* The instinct was right — something was overloading — and the convolver was in the chain, but the cause was **what was being put into everything**. Measured: **84% of the score's energy by RMS sat under 60Hz**, and its spectral centroid while walking was 147Hz. The drone's lowest voice is the root two octaves down, which for these keys is 20–31Hz; the throb's fundamental is 45Hz sagging to 39. None of that is audible on any speaker this will be played on, all of it drove the ceiling, and so every peak of a rumble nobody could hear pulled the part they *could* hear down with it. It got worse with intensity because saturating a sine leaves the fundamental exactly where it was and adds to everything above it. That is the dropout, and it is why it happened on every machine. Four things. Two highpasses at 42Hz and a shelf taking **8dB off everything under 120Hz**; a 130Hz highpass in front of the reverb, because no room reverberates usefully down there and a six-second tail fed sub-bass is an integrator; the drone's voices and the throb's fundamental turned down at source, with the throb leaning on its saw instead; and then the output turned *up*, because the headroom that freed was the whole point. Also fixed on the way, and it was a real fault of the same family: the ceiling was a DynamicsCompressor at −6 and 9:1, and Chrome's applies no makeup, so over a sevenfold range of drive the peak never moved off 0.35 — a limiter holding the whole thing down rather than catching peaks. It is a waveshaper now, which has no attack, no release and no memory and therefore cannot take a quiet moment away because a loud one came before it, with a gentle compressor in front of it as a shock absorber. Distortion is level-compensated now as well — a makeup behind each drive — so turning the dirt up changes the timbre instead of the level, and the amounts are down by about half as asked. The glissando is a little under half its old level. Measured, seed 424242, walking: **centroid 147→428Hz, energy under 60Hz 0.77→0.71, time on the ceiling 0.46%→0.01%**, and the audible part of the score is now most of it. Two new checks: the centroid has a *floor* as well as a ceiling, and the output may not sit on the clipper. |
| 2026-08-29 | **rushing pinned the picture at the top of everything** | fixed | Played: *"it is still too intense most of the time and occasionally it is blowing up. This might be because I am sometimes rushing through the house."* The guess was right and it names the mechanism. Every term the score adds to the perception pass is **additive**, and they were written as flashes between notes — but rushing fires a bell in every room and a swell at every door on top of a throb that never stops, and flashes that overlap are not flashes, they are a plateau. Measured over twelve seconds, mean of what the picture was being handed: walking pulse 0.15 / weight 0.22, **rushing 0.35 / 0.52**. Three changes. The flash decays properly now — at a half-life of 126ms a throb every 200ms never let go, so it is 63ms, and the weight under it 1.5× faster. The amounts come down to roughly the `?fx=0.6` that had been reported as about right: warp 0.55/0.30 → 0.34/0.16, blur 2.1 → 1.3, snow 0.060/0.075 → 0.038/0.050, saturation 0.35 → 0.22, vignette 0.085/0.22 → 0.055/0.15, and the lamps 0.09/0.34 → 0.06/0.26. And a **governor**: the whole set is summed and scaled back in proportion past a fixed ceiling, so a busy moment is *denser* than a quiet one rather than brighter — measured under a rush it asks for 0.85 and is given 0.62, with a check on it. Same rush after: walking 0.09 / 0.13, rushing 0.29 / 0.31, and the extra warp while rushing is 15% where it was 35%. **The other half of the report is not reproduced**: rushing does not make the score louder. Offline, fifteen seconds with forty room changes, twenty doors and thirteen impacts against the same passage with none of them — peak 0.338 vs 0.344, RMS 0.145 vs 0.152, no sample within 5% of the ceiling in either, and the limiter worst −5.9dB. If "blowing up" is a sound rather than a picture then it is not the score's level and the next place to look is the foley, which fires an impact per collision at up to 0.46 and predates all of this. |
| 2026-08-29 | **the pass and the score are one thing** | **for Mick to judge** | Two notes from the same playtest. *"The full screen shader seems more extreme than it did. Is this possible?"* — yes, and it was a fault rather than a feeling: the five terms the score adds to the perception pass were **not multiplied by the pass's own gain**, so `?fx=N` was a dial that no longer controlled the whole thing. The largest of them are a note pushing the warp by 55% and a flicker adding 0.22 to the vignette and taking 34% off every lamp, and none of that existed a build ago. All five now scale with `gain`, so the dial means what it says again and the pass can be turned back down to where it was. Then, Mick's second note: *"it would be great if the music could respond to the shader intensity by becoming more distorted."* It does now. The score is handed how hard the pass is actually working — depth times `?fx=N` — and the *difference* between that and depth alone drives throb drive, grit, crackle and the wow across the whole mix, so the default changes nothing and turning the pass up dirties the music with it. Measured as crest factor, which is what distortion actually is: 2.26 at `?fx=1` down to 1.94 at `?fx=3` on seed 424242, and monotonic across the five seeds. Spectral centroid was the obvious measure and it is the wrong one — driving a sub-bass into a shaper adds as much weight underneath as edge on top, and on seed 7 a dirtier score measured as a *darker* one, which is how the first version of the check came to fail. |
| 2026-08-29 | **the score dropped out if you ran** | fixed | Reported from play: *"the music track doesn't seem stable. Sometimes it simply drops out. If you run quickly from one room to another, it builds up a bunch of pads and then I think the whole thing tops out and kills the output."* Exactly right, and two causes. The organ **sustained for twelve seconds and nothing stopped the last one** — attack 2.4s, hold a bar and a half, release 6.0 — while a new one could start every bar (2.7s) plus one per door, so running through the house stacked five or six of them and the limiter took the whole score away trying to hold them. There is now only ever one: a new chord crossfades the last out over 0.9s, and a chord change may not *announce* itself more than once every 3.2s, though it still governs the bells and the stabs immediately — the chord follows you into every room, the organ does not. And every throb and every stab **built a WaveShaperNode with a 2048-point curve computed in JavaScript**, several times a second, which is a great deal of garbage to be making while a game is trying to hold a frame; drive is now a gain in front of one fixed curve per voice type, which is how a distortion pedal does it anyway and sounds better besides, because all the voices distort into each other. Measured over eighteen seconds of running room to room with doors, on the build that was played against this one: **1004 oscillators and 94 waveshapers → 625 and 0**, and the limiter's worst reduction −8.3dB → −1.5dB. The counters are the fix as much as the fix is: `VK.music.info()` now reports `pads`, `made` and `squash`, and there are three checks on them, because "it drops out sometimes" is not something you can chase without a number. |
| 2026-08-29 | **the score, second pass — the arpeggiator was the problem** | **for Mick to judge** | Played: *"The arppegiator is just annoying and has to go altogether. The overall atmosphere is relaxing and not disturbing which is wrong for the feel. The low drone is not distorted or crackling enough, and there's no sense of unease, flicker or suspense. The chords have some pretty boring major chords thrown in and it's not clear why. Nothing has enough reverb. The form is really great though."* Six faults and one thing to protect, and the first one explains most of the second: **a figure you can follow is a thing to enjoy**, and an arpeggiator turned the building into a nice place to be. It is gone and nothing melodic replaced it. What walking drives now is a **throb** — one distorted low note, always the root, always the same octave, sagging in pitch as it decays the way a tape does — and a **stutter gate** on the drone. Every eighth footstep still re-rolls both patterns, so the form survives intact: what you are hearing is something you walked into being, it just is not a tune any more. **The majors were diatonic thirds.** Stacking `[0,2,4]` over a minor mode hands you bIII, bVI and bVII, which are three major triads arriving for no reason anybody in the building could explain — the fault was that I used a mode at all. There is no scale now: ten chord *shapes*, all minor or dissonant (root against its own flat second, half-diminished, a diminished triad a tritone away, bVI taken as a minor and never the major), and `info().majors` is 0 with a check on it. The drone carries a permanent semitone or tritone rub, chosen per house, and there is no third in it at all — nothing to hear as major or minor, only as wrong. **Dirt**: an asymmetric waveshaper on the drone and a second on the throb, both driven harder by depth and by how much you are looking about; irregular crackle rather than hiss, drawn as sparse impulses so it ticks instead of hissing; two amplitude LFOs at unrelated rates so it never settles. **Flicker and suspense**, which were simply absent: a stutter gate that chops the drone in bursts at a rate unrelated to the tempo, holes in the whole mix a fifth of a second wide, and the rising glide made a regular device rather than a deep-rooms-only one — a third of them **cut off part-way up**, which is the difference between suspense and a build. And the flicker takes the lights with it: the stutter in the drone and the bad bulb in the ceiling are the same event, and nothing explains either. **Reverb**: one convolver at 3.4s with sends around 0.2 was far too dry. 6.0s, a 45ms pre-delay, nine sparse early reflections at the front, and sends of 0.3 to 1.0 with a return that opens further the deeper you are. Found on the way, and it is the sort of thing this project keeps finding: the asymmetric shaper had **a DC offset**, so silence was not silent and the limiter was working on nothing. Inaudible; caught only because there is a check that measures the peak of a passage that is supposed to be zero. Measured, seed 424242, 8s: silent 0.000 / 0.0000, standing 0.122 / 0.065 / 81Hz, walking 0.305 / 0.111 / 253Hz, deep 0.344 / 0.178 / 260Hz — walking is 1.7× standing by RMS, and the check now uses RMS because the drone is saturated and its *peak* barely moves however quiet it gets. Per-room draw calls, triangles, programs and lights identical to the previous build in every room of every seed; pixel A/B 0.02%, which is the same-build control on this machine. |
...
| 2026-08-28 | **a crossing is silent** | fixed | The portal traversal sound is gone. Everything else about a crossing is built so you do not notice it — the transform is rigid so you keep your height above the floor, the view through is now developed exactly like the room around it, the far side is already drawn before you arrive — and then it played a thump to announce that something had happened. It was the one thing giving the fold away. If a player is meant to walk from one room into another and work out only later that they cannot have, the crossing has to be silent. Checked, so it cannot come back. |
| 2026-08-28 | **the project is called Wittgenstein 3D** | fixed | It always was. "Vitkinstein" was a speech-to-text error that got as far as the title screen, the README, the specification and the package name, and stayed there because it was in every file that would have told anyone otherwise. Renamed everywhere in the repository, and — on Mick's say-so, because it changes the address the game is played at — the GitHub repository too. It is now `micknoise/wittgenstein3d`, and the game is at **https://micknoise.github.io/wittgenstein3d/**. GitHub redirects the old address, but the canonical one has moved. The working directory on Mick's machine is still named for the error; renaming that is his to do. |
| 2026-08-28 | **sound: a tin can is not a bell, and a drum is not a bucket** | **for Mick to judge** | *"The cup and the tin can are still too resonant. They don't resonate. They are tiny. The oil drum is too high pitched. It is deep and needs internal reverberation."* One metal bank could not cover a tin can and an oil drum, and it was tuned between the two, so both were wrong. Three sizes now: **tin** 1067Hz and 31ms, a clatter that stops; **metal** for pipes and buckets, 257Hz and 342ms; **drum** at 121Hz over half a second, with the inside of it baked in — two short comb delays at 11.3 and 17.9ms fed back into the sample at load, which is the enclosed air and is the entire difference between a drum and a sheet of steel. The cup came down to 28ms and is now almost all noise. |
| 2026-08-28 | **sound: quieter, and the brick was a bell** | **for Mick to judge** | Four things. A dropped **brick** sounded like a bell because it is drawn with the rust texture and the sound class was inferred from the visual material — so it was playing the metal bank. Same class of error had the mug on `soft` and the ball on `ceramic`. Props can now say what they sound like, overriding what they look like. **Glass** was a struck bell: 280ms of ring at 1220Hz, now a 50ms chink at 1080Hz. **Everything muted** — master 0.9→0.62, impacts to about half, footsteps 0.5→0.30, metal's ring 1226→429ms. The 'ping' on grab was a tuned 320Hz burst, which is a notification sound; it is a 25ms click now. And the **room tone is gone**, which is the noise that was under everything all the time — it has followed the hum out. When nothing is happening, nothing is happening. |
| 2026-08-28 | **sound: pitched down** | **for Mick to judge** | *"The wood sounds like a child's woodblock... the metal, particularly footsteps on metal, sounds like a child's glockenspiel without the decay. Too high again."* Three causes, and the largest was not in the sound design at all: the playback rate reached **1.90** for anything small, so a mug and a brick were both played most of an octave above the pitch they were drawn at. Narrowed to 0.60–1.18. Then the drawn sounds themselves: wood was a clean 190Hz partial over a click, which is a woodblock — an instrument, built to be tuned and heard — now 104Hz and 157Hz set close together so neither reads as a note, under a much heavier broadband thock. Metal came down an octave and a half, gained grit, and its tail went from 0.35s to 1.2s, which is the other half of "without the decay". And the tile *footstep* had a 640Hz sine in it, which is a glockenspiel bar — that was the metal footstep. Measured as spectral centroid: wood 246→150Hz, metal 531→206Hz, tile 308→154Hz, ceramic 856→591Hz. Glass deliberately stays bright at 1172Hz. |
| 2026-08-28 | **sound: drawn, not simulated** | **for Mick to judge** | *"the core sound creation method is more of a simulation than a creative treatment of the problem... the sounds just sound like bad syndrums."* Correct, and it kills two passes of work. Modal synthesis is physically right and produces a syndrum, because what a recording of a dropped mug mostly contains is the things it does not model — the scrape, the rattle after, the room — and it cost a filter bank per hit. Sounds are now **drawn once at load into buffers**, the same way textures are drawn onto canvases at load, and playing one is a single node. Foley rather than physics: louder, shorter and more characterful than the real thing. Ten banks, three takes each, plus pitch either way, because the giveaway is never the sound, it is hearing it twice. The convolution reverb went with it — it was the CPU complaint and it was my idea, not a request. Occlusion stays: one filter per sound, and it was the cheapest cue anyway. 228ms to draw the lot, once, on the click that starts the game. |
