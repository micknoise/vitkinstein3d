...
| **F38a** | The whole frame is footage from a camera strapped to somebody's head: a wide lens, scan lines, chroma that does not sit on the luma, interference in bursts, and a head-switching bar rolling up the picture every few seconds. | Watch the frame rather than the room for ten seconds. `?novhs=1` shows the raw render. |
