...
| **F39a** | Every interaction varies it. Walking is why the throb exists at all, and every eighth footstep re-rolls the throb and flicker patterns. Taking an object gives you a note that sustains until you put it down, and throwing it ends that note — the throw itself is silent, because the object hits something a moment later and *that* lands in the bar. An impact is an accent quantised to the next sixteenth; a door moves the harmony. Looking about quickly opens the grit, quickens the flicker and brings the rub up. | Walk a while, then take something, carry it, throw it, and open a door. |
...
| **F39i** | The score puts its energy where it can be heard. Nothing under 42Hz reaches the master at all, everything under 120Hz is 8dB down, and nothing under 130Hz goes into the reverb. Its spectral centroid while walking is above 210Hz — under the foley, but inside the audible band. | `npm run music` prints centroid and the share of energy under 60Hz for each state. |
| **F39l** | Nothing in the score interrupts a ramp by reverting to where it started. A parameter that is cancelled mid-ramp holds the value it has actually reached; a gate never steps a sustained sound instantaneously; and the score's own held breath ducks the layers rather than muting the master, so the room it was in rings on through it. | Leave a room while the organ is still arriving — it should fade, not cut. |
| **F39m** | A collapse in the output is recorded rather than argued about. The score listens to its own output and, when the level falls away while it believes it is playing, writes down everything that was true at that moment. | `VK.music.drops`, and a console warning as each one happens. |
| **F39k** | The reverb's impulse response is a decaying noise burst with its mean removed and a 90Hz highpass over the finished response, normalised to unit energy with the browser's own normalisation turned off. It has no DC and no infrasonic wander: its gain below hearing is under half its gain in the band it reverberates. | `VK.music.ir` is the buffer; the test suite measures its DC against its mid-band magnitude. `?noverb=1` takes the reverb out entirely, leaving everything else as it is. |
| **F39j** | It does not overload. The ceiling is a waveshaper, not a compressor: it has no attack, no release and no memory, so a loud moment cannot take away a quiet one that follows it. Distortion changes the timbre and not the level — each drive has a makeup behind it. The output sits on the ceiling less than 1% of the time in any state. | `npm run music` prints `clip`; the test suite checks it. |
| **F39h** | What the score does to the picture is bounded. The terms are additive and were written as flashes between notes, so a player rushing through the building — a bell in every room, a swell at every door, over a throb that never stops — would otherwise hold the picture near the top of all of them at once. A governor scales the whole set back in proportion past a fixed ceiling, so a busy moment is denser than a quiet one rather than brighter. | `VK.music.info()` reports `push` and `pushMax`: what the score asked for and what it is allowed. Run from room to room slamming doors. |
